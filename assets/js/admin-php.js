(() => {
  ...
  const renderPropertyList = () => {
    ...
    return `<div class="content-list">${state.properties.map(item => {
      ...
      return `<article class="content-row">
        ...
        <div class="row-actions"><button type="button" data-edit-property="${escapeHtml(item.slug)}">Editar</button><button type="button" data-delete-property="${escapeHtml(item.slug)}">Eliminar</button></div>
      </article>`;
    }).join('')}</div>`;
  };
  ...
  qs('[data-property-form]')?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      hydrate(data);
      setStatus(data.message + ' data/properties.json actualizado.');
  ...
  qs('[data-rebuild-site]')?.addEventListener('click', async () => {
    try {
      const fd = new FormData();
      fd.append('domain', 'https://inversionesdavar.com/');
      setStatus('Regenerando páginas del blog y sitemap...');
