...
blog_en=json.load(open(ROOT/'data/blog-en.json',encoding='utf-8')) if (ROOT/'data/blog-en.json').exists() else (json.load(open(ROOT/'data/blog.json',encoding='utf-8')) if (ROOT/'data/blog.json').exists() else [])
blog_es=json.load(open(ROOT/'data/blog-es.json',encoding='utf-8')) if (ROOT/'data/blog-es.json').exists() else []
for b in blog_en + blog_es:
    if b.get('status','published')!='draft': canonical_paths.append('/'+b.get('url',f"blog/{b['slug']}/").lstrip('/'))
canonical_paths += ['/blog/','/es/blog/']
seen=[]
...
# Final portability pass: keep canonical/hreflang absolute, but make local assets and
# navigation work from a custom-domain root, GitHub project path or XAMPP subfolder.
from path_compat import patch_project_paths
patched_files, patched_urls = patch_project_paths(ROOT)
print('Path compatibility:', patched_files, 'HTML files,', patched_urls, 'URL values updated')
